...
报告会输出次日回访率、7 日平均记录天数、体重填写率、上下文标签填写率、Dashboard / Trends 使用率、报告入口曝光率、付费意愿点击率、点击 / 曝光转化率、alpha 反馈提交率、`decision` 和 `decisionReview`。`continue_candidate` 只代表数据达标，`decisionReview.recommendation` 仍会检查真机证据、用户原话和竞品真机样本。
