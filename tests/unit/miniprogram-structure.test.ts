...
describe("miniprogram structure", () => {
  ...
  it("calls the existing backend through bearer-token API helpers", () => {
    ...
    expect(dashboardMarkup).toContain("今天先看");
    expect(dashboardMarkup).toContain("体重线索");
    expect(dashboardMarkup).toContain("今天三项");
    ...
    expect(trendsMarkup).toContain("这条线在说");
    expect(trendsMarkup).toContain("体重线");
    expect(trendsMarkup).toContain("体重线索");
    expect(trendsMarkup).toContain("最近落点");
