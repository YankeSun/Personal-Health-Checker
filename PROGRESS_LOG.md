...
- 校正 `8746d81-qwen` 的趋势图细节：补录 tooltip 原先把数值单位写成了占位词“单位”，现已改为显示真实单位，避免趋势阅读时产生歧义。
- `阶段 C2` 已推进：目标系统改成统一的行为化表达，设置页会直接说明“系统会怎样判断达标”，Dashboard / Trends / Reminder 也统一改成同一套目标语言。
- `阶段 C3` 继续推进：Dashboard 和 Trends 开始显示“距离目标还差多少 / 是否已经落在目标区间内”，用户不只知道有没有达标，也知道偏差方向和幅度。
...
- `阶段 C3` 继续推进：Today 页开始在输入过程中直接显示“今天会怎样判断 / 还差多少 / 是否已经对齐目标”，用户不用先保存再跳到 Dashboard 才知道当前记录离目标还有多远。
- `阶段 C3` 继续推进：History 页新增月度小结，开始直接告诉用户这个月的记录节奏和上个月相比是否更稳、以及哪一项最容易漏记，回看时不再只是翻日历和表格。
